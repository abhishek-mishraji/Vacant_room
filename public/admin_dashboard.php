...
require_once __DIR__ . '/../admin/auth.php'; // will start session and check admin
...
// if building chosen, load rooms + statuses for that day/slot
$rooms_by_floor = [];
if ($building_id) {
    $stmt = $conn->prepare("
        SELECT r.room_id, r.floor_no, r.room_number, COALESCE(s.status,'Vacant') AS status
        FROM rooms r
        LEFT JOIN schedules s ON s.room_id = r.room_id AND s.day_of_week = ? AND s.slot_id = ?
        WHERE r.building_id = ?
        ORDER BY r.floor_no DESC, r.room_number
    ");
    $stmt->execute([$day, $slot, $building_id]);
    $rows = $stmt->fetchAll();
    foreach ($rows as $row) {
        $rooms_by_floor[$row['floor_no']][] = $row;
    }
}
...
<div class="admin-dashboard">
    ...
    <div class="admin-filter-panel">
        <form method="get" class="admin-form">
            <div class="form-group">
                ...
                <select id="building-select" name="building_id" class="form-control" required>
                    <option value="">--Select Building--</option>
                    <?php foreach ($buildings as $b): ?>
                    <option value="<?= $b['building_id'] ?>" <?= $b['building_id'] == $building_id ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <select id="slot-select" name="slot" class="form-control">
                    ...
                    <option value="<?= $k ?>" <?= $k == $slot ? 'selected' : '' ?>><?= $v ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <div class="empty-message">
        ...
        <p>Please select a building to manage room statuses.</p>
    </div>
    <?php else: ?>
    <div class="building-info">
        <strong>Currently Editing:</strong>
        <?= htmlspecialchars($buildings[array_search($building_id, array_column($buildings, 'building_id'))]['building_name'] ?? "Building $building_id") ?>
        — <?= htmlspecialchars($day) ?>, <?= $slots[$slot] ?>
    </div>

    <?php if (empty($rooms_by_floor)): ?>
    <div class="empty-message">
        <p>No rooms found for this building. Please check your selection or add rooms to this building.</p>
    </div>
    <?php endif; ?>

    <?php foreach ($rooms_by_floor as $floor_no => $floorRooms): ?>
    <div class="floor-container">
        <div class="floor-header">
            <h3 class="floor-title">Floor <?= htmlspecialchars($floor_no) ?></h3>
        </div>
        <div class="rooms-container">
            <?php foreach ($floorRooms as $room): ?>
            <div class="room-card <?= strtolower($room['status']) ?>">
                <?php if (isset($_GET['saved_room']) && $_GET['saved_room'] == $room['room_id']): ?>
